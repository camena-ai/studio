...
- **pnpm policies.** Lifecycle scripts run only for packages listed under `allowBuilds` (pnpm fails the install on any undecided script; decide `true` or `false`, never approve blindly). pnpm's default minimum release age (24 h) applies: pin versions at least a day old rather than adding `minimumReleaseAgeExclude` entries. Set `ELECTRON_SKIP_BINARY_DOWNLOAD=1` when you do not need the Electron binary (CI does).
...
- **Contracts.** `@studio/contracts` is pinned to an exact published version. Bumping it is its own commit that names the contract change it adopts.
...
Feature branches from `main`, fast-forward merged back. Worktrees are created under `.claude/worktrees/` (git-ignored). Run git commands from the worktree you are in; operate on the primary checkout with `git -C <path>` rather than `cd`. Changes that adopt a new `@studio/contracts` version or a new Electron major are their own commits.
